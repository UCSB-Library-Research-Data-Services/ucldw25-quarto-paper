---
title: "Wanna Play? A study on cats' fetching behaviors"
author: Kat Whiskerstein
format: html
...
@biasi2024 determined the cat-human interaction from the viewpoint of classical mechanics theories and concepts. Beyond being an enjoyable model to exemplify physics, findings such as can be used to better understand cats fetching habits as well, including his considerations on Newton's mechanics in the presence of an external potential, which can be applied as $V_{cat}^{(𝛿)}$, as illustrated in @fig-physics.

![Interpretation of the cat–human interaction potential. In (a), the cat is attached to the person, while in (b) the person is a complete stranger to the cat. In both cases, the cat is illustrated at rest at three equilibrium points.](img/cat-physics.jpeg){#fig-physics fig-align="center" width="200"}

Previous research have also suggested a few main themes associated with feline fetching habits. @forman2023 presents a concept map (@fig-concept) which illustrates the relationship and branching of sub-themes associated with the fetching process itself, the acquisition or retrieval process, and the conditions in which the fetching session happens.

![Thematic map illustrating key themes associated with feline fetching habits.](img/cat-fetching-themes.png){#fig-concept}
...
A survey was distributed to 822 cat parents in the region to better understand the frequency, conditions, and factors influencing their cats' tendency to fetch[^1]. The survey was designed to capture a broad range of data on various aspects of feline fetching behavior, from the cat's age and breed to the frequency of play and engagement in fetching activities.

[^1]: This paper is a fictional work created solely for instructional purposes and is not based on actual research. While it may reference ongoing research in the field, all content within this document, including the underlying dataset, is fabricated and based on synthetic data. The primary objective of this paper is to demonstrate how [RStudio and Quarto](https://quarto.org/docs/get-started/hello/rstudio.html) can be used effectively to streamline and enhance reproducible scientific writing. This paper is intended as a learning tool for understanding the integration of these platforms in the context of research workflows. Please do not use or cite this paper.
...
1.  **Cat’s Age**: The age of the cat (categorized into young, adult, or senior).

2.  **Breed**: The breed of the cat, with particular interest in breeds commonly associated with fetching behavior (e.g., Maine Coon, Bengal).

3.  **Fetching Frequency**: The frequency with which the cat engages in fetching (e.g., never, occasionally, frequently).

4.  **Play Duration**: The typical duration of playtime for the cat in a single session.

5.  **Fetch Motivation**: The types of objects used for fetching (e.g., balls, toys, sticks).

6.  **Owner’s Interaction**: The level of interaction or training by the owner to encourage or discourage fetching behavior.

7.  **Cat’s Health and Activity Level**: Whether the cat has any health issues and their general activity level (e.g., low, moderate, high).

8.  **Environmental Factors**: Whether the cat has access to outdoor play or is an indoor-only cat.
...
#| echo: false
# Load the data
  catfetching <- read.csv("data/cat-fetching.csv")
# Load library
  library(ggplot2)
```

Our sample included cats from 822 unique pet parents, indicating an average of 1.9 cats per respondent, with a maximum of `r max(catfetching$household)`. Out of a total of 1543 cases, 790 were males (51.2%), while 753 females (48.8%) account for the second-largest group. The average age of the cats included on the dataset was `r round(mean(catfetching$age), 1)`.
...
#| echo: false
#| warning: false
#| fig.cap: "Fetching average time per breed."
# Bar chart for Fetching average time per Breed
# Code for bar chart
  ggplot(catfetching, aes(x = reorder(breed, -average_hours_per_week), y = average_hours_per_week, fill = breed)) + stat_summary(fun = "mean", geom = "bar", position = "dodge", width = 0.7, alpha = 0.7) + labs(title = "",
       x = "breed", 
       y = "hours/week",
       fill = "Breed") +
  scale_fill_brewer(palette = "Set3") +  # Choosing colors
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 5)) +  # Set y-axis
...
#| echo: false
#| warning: false
#| fig.cap: "Average time spent per week by preferred toy and sex."
...
ggplot(catfetching, aes(x = preferred_toy, y = average_hours_per_week, fill = sex)) +
  geom_boxplot(alpha = 0.6) +  labs(title = "", 
       x = "toy", 
       y = "hours/week", 
       fill = "Sex") +
  scale_y_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 5)) +  # Set y-axis
